...
/**
 * recives data about a form field and spits out the personal html 
 *
 ...
 */
function personal_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		// image
		case 'image':
			$image = get_template_directory() . '/inc/images/image.png'; 	
		...
		case 'repeatable':
			...
			foreach( $meta as $row ) {
				...
				foreach ( $repeatable_fields as $repeatable_field ) {
					...
					echo personal_meta_box_field( $repeatable_field, $meta[$i][$repeatable_field['id']], array( $id, $i ) );
					echo '</p>';
				} // end each field
 ...
 */
function personal_meta_box_find_field_type( $needle, $haystack ) {
	foreach ( $haystack as $h )
		if ( isset( $h['type'] ) && $h['type'] == 'repeatable' )
			return personal_meta_box_find_field_type( $needle, $h['repeatable_fields'] );
 ...
 * @return	bool				whether or not the type is in the provided array
 */
function personal_meta_box_find_repeatable( $needle = 'repeatable', $haystack ) {
	foreach ( $haystack as $h )
 ...
 */
function personal_meta_box_santitize_boolean( $string ) {
...
/**
 * outputs personally sanitized data
 *
 ...
 */
function personal_meta_box_sanitize( $string, $function = 'sanitize_text_field' ) {
	switch ( $function ) {
		case 'intval':
 ...
 */
function personal_meta_box_array_map_r( $func, $meta, $sanitizer ) {
	...
	foreach( $meta as $key => $array ) {
		...
		foreach( $array as $arrayKey => $arrayValue )
			if ( is_array( $arrayValue ) )
				$array[$arrayKey] = personal_meta_box_array_map_r( $func, $arrayValue, $newSanitizer[$arrayKey] );
...
class personal_Add_Meta_Box {
	...
	function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) && in_array( get_post_type(), $this->page ) ) {
			// js
			$deps = array( 'jquery' );
			if ( personal_meta_box_find_field_type( 'date', $this->fields ) )
				$deps[] = 'jquery-ui-datepicker';
			if ( personal_meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jquery-ui-slider';
			if ( personal_meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			if ( in_array( true, array(
				personal_meta_box_find_field_type( 'chosen', $this->fields ),
				personal_meta_box_find_field_type( 'post_chosen', $this->fields )
			) ) ) {
				wp_register_script( 'chosen', get_template_directory() . '/inc/js/chosen.js', array( 'jquery' ) );
				$deps[] = 'chosen';
				wp_enqueue_style( 'chosen', get_template_directory() . '/inc/css/chosen.css' );
			}
			if ( in_array( true, array( 
				personal_meta_box_find_field_type( 'date', $this->fields ), 
				personal_meta_box_find_field_type( 'slider', $this->fields ),
				personal_meta_box_find_field_type( 'color', $this->fields ),
				personal_meta_box_find_field_type( 'chosen', $this->fields ),
				personal_meta_box_find_field_type( 'post_chosen', $this->fields ),
				personal_meta_box_find_repeatable( 'repeatable', $this->fields ),
				personal_meta_box_find_field_type( 'image', $this->fields ),
				personal_meta_box_find_field_type( 'file', $this->fields )
			) ) )
				wp_enqueue_script( 'meta_box', get_template_directory() . '/inc/js/meta-scripts.js', $deps );
			
			// css
			$deps = array();
			wp_register_style( 'jqueryui', get_template_directory() . '/inc/css/jqueryui.css' );
			if ( personal_meta_box_find_field_type( 'date', $this->fields ) || personal_meta_box_find_field_type( 'slider', $this->fields ) )
				$deps[] = 'jqueryui';
			if ( personal_meta_box_find_field_type( 'color', $this->fields ) )
				$deps[] = 'farbtastic';
			wp_enqueue_style( 'meta_box', get_template_directory() . '/inc/css/meta_box.css', $deps );
		}
	}
	...
	function add_box() {
		foreach ( $this->page as $page ) {
			add_meta_box( $this->id, $this->title, array( $this, 'personal_meta_box_callback' ), $page, 'normal', 'high' );
		}
	}
	 ...
	 */
	function personal_meta_box_callback() {
		// Use nonce for verification
		wp_nonce_field( 'personal_meta_box_nonce_action', 'personal_meta_box_nonce_field' );
		...
		foreach ( $this->fields as $field) {
			if ( $field['type'] == 'section' ) {
				...
				echo '<tr>
						...
						echo personal_meta_box_field( $field, $meta );
	...
	function save_box( $post_id ) {
		...
		// verify nonce
		if ( ! isset( $_POST['personal_meta_box_nonce_field'] ) )
			return $post_id;
		if ( ! ( in_array( $post_type, $this->page ) || wp_verify_nonce( $_POST['personal_meta_box_nonce_field'],  'personal_meta_box_nonce_action' ) ) ) 
			return $post_id;
		// check autosave
		...
		foreach ( $this->fields as $field ) {
			...
			if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
				...
				if ( isset( $new ) && '' == $new && $old ) {
					...
					if ( is_array( $new ) )
						$new = personal_meta_box_array_map_r( 'personal_meta_box_sanitize', $new, $sanitizer );
					else
						$new = personal_meta_box_sanitize( $new, $sanitizer );
