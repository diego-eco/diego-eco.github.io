...
?>
        ...
        <div class="notice notice-error uct-notice is-dismissible">
            <p>
                <?php printf( esc_html__( 'Please use a %s child theme to make changes', 'personal' ), $this->theme->get( 'Name' ) ); ?>
                <a class="uct-activate" href="javascript:;"><?php esc_html_e( 'Activate now', 'personal' ); ?></a> 
            </p>
        </div>
<?php
        }
        ...
        function activate_child_theme() {
            ...
            $settings = get_option( 'theme_personals_' . $this->child_slug );

            if ( false === $settings ) {
                $parent_settings = get_option( 'theme_personals_' . $parent_slug );
                update_option( 'theme_personals_' . $this->child_slug, $parent_settings );
            }

            wp_die( esc_html__( 'All done!', 'personal' ) );
        }
        ...
        function create_child_theme() {
            ...
            if ( wp_mkdir_p( $child_dir ) ) {
                ...
                wp_die( esc_html__( 'Error: theme folder not writable', 'personal' ) );
            }
        }
